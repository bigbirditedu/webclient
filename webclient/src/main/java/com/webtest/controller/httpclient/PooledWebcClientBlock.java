 ...
 */
@RestController
public class PooledWebcClientBlock {
    private final String url = "http://127.0.0.1:8081";
    private WebClient webClient;

    public PooledWebcClientBlock() {
    ...
    @GetMapping("pooledblockwebc")
    public List<Product> getProductsWebclient() {
        long start = System.currentTimeMillis();
        Flux<Product> productFlux = webClient.get().uri("/products?type=pooledblockwebc").retrieve().bodyToFlux(Product.class);
        List<Product> productList = productFlux.collectList().block();
        long end = System.currentTimeMillis();
        System.out.println(Thread.currentThread().getName() + " pooled block webclient cost:" + (end - start));
        return productList;
    }
}
